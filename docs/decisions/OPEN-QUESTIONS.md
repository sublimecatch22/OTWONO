...
| **OQ-25** | Streaming a fan-out fetch straight to a file. `fetch_object_from_peers` assembles the whole object in RAM before anything is written, so a T0 board with 512 MiB cannot fetch a 2 GiB object however the result is delivered — ADR-0018 moves the *delivery* to a file and does not change this. Write chunks to the export file as they arrive and verify in place, or keep a bounded window in memory? The first is better and makes a partially-written file a thing that exists, which the label rules have nothing to say about yet | Phase 6 | Phase 6, after ADR-0018 lands |
