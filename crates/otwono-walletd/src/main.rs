...
require confirmation. ADR-0024 built that channel, but only a designated confirmer may
answer and the shipped image designates nobody -- so on a booted node today the wallet can be
